...
describe 'my own map' do
  it "returns an array with all values made negative" do
    expect(map_to_negative([1, 2, 3, -9]){|n| n * -1}).to eq([-1, -2, -3, 9])
  end
  ...
  it "returns an array with the original values multiplied by 2" do
    expect(map_double([1, 2, 3, -9]){|n| n * 2}).to eq([2, 4, 6, -18])
  end

  it "returns an array with the original values squared" do
    expect(map_squared([1, 2, 3, -9]){|n| n * n}).to eq([1, 4, 9, 81])
  end
end
...
describe 'my own reduce' do
  it "returns a running total when not given a starting point" do
    source_array = [1,2,3]
    expect(reduce_total(source_array){|memo, n| memo + n}).to eq(6)
  end

  it "returns a running total when given a starting point" do
    source_array = [1,2,3]
    starting_point = 100
    expect(reduce_total(source_array, starting_point){|memo, n| memo + n}).to eq(106)
  end
